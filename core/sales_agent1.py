...
Available tools: rag, web_search, payment (and Zapier tools if listed above)
...
Use a Zapier tool when:
- User action requires saving contact info to a CRM (e.g. HubSpot, Salesforce)
- User wants to receive a follow-up email or Slack message
- User wants to schedule a meeting or create a calendar event/task
- Stage is CLOSING or POST_SALE and there is contact info available to save
- NEVER use Zapier speculatively — only when user intent clearly maps to an automation
  ...
  "message_type": "text"
    ...
    async def _execute_parallel(self, tools: List[str], query: str, analysis: Dict, user_id: str = None, **kwargs) -> Dict[str, Any]:
        ...
        for i, tool in enumerate(tools):
            if tool in self.available_tools or tool in self._zapier_tools:
                ...
                # Dispatch: Zapier tools vs standard tools
                if tool in self._zapier_tools:
                    zapier_args = analysis.get("tool_args", {}).get(tool, {})
                    if not zapier_args.get("instructions"):
                        zapier_args = {"instructions": query}
                    logger.info(f"🔌 {tool.upper()} #{count} ZAPIER ARGS: {zapier_args}")
                    task = self.tool_manager.execute_zapier_tool(tool, zapier_args)
                else:
        ...
        if tasks:
            ...
            for tool_name, result in zip(keys, raw_results):
    ...
    async def _execute_sequential(self, tools: List[str], query: str, analysis: Dict, user_id: str = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            if first_tool_name in self._zapier_tools:
                zapier_args = analysis.get("tool_args", {}).get(first_tool_name, {})
                results[first_tool_key] = await self.tool_manager.execute_zapier_tool(first_tool_name, zapier_args)
            else:
        ...
        for i in range(1, len(order)):
            ...
            try:
                # Zapier tools: skip middleware, build instructions from previous results + original query
                if current_tool_name in self._zapier_tools:
                    # Check if analysis already provided tool_args with instructions
                    zapier_args = analysis.get("tool_args", {}).get(current_tool_name, {})

                    # If instructions not populated, build them from previous results + query
                    if not zapier_args.get("instructions"):
                        previous_context = []
                        for prev_key, prev_result in results.items():
                            if isinstance(prev_result, dict) and prev_result.get("success"):
                                retrieved = prev_result.get("retrieved", "")
                                if retrieved:
                                    previous_context.append(retrieved[:2000])
                        context_str = "\n\n".join(previous_context) if previous_context else ""
                        zapier_args = {
                            "instructions": f"{query}\n\nContext from knowledge base:\n{context_str}" if context_str else query
                        }

                    logger.info(f"   → Step {i+1}: Executing {current_tool_key.upper()} (Zapier — skipping middleware)")
                    results[current_tool_key] = await self.tool_manager.execute_zapier_tool(current_tool_name, zapier_args)

                else:
                    # Standard tools: use middleware as before
    ...
    def _format_tool_results(self, tool_results: dict) -> str:
        ...
        for tool, result in tool_results.items():
            if isinstance(result, dict):
                ...
                # Handle Zapier tool results
                if tool in self._zapier_tools or result.get('tool_name') in self._zapier_tools:
                    if result.get('success'):
                        zapier_result_text = result.get('result', 'Action completed successfully')
                        formatted.append(f"{tool.upper()} (ZAPIER):\n{zapier_result_text}")
                        logger.info(f"✅ Zapier tool result formatted: {tool}")
                    else:
                        error_msg = result.get('error', 'Zapier action failed')
                        formatted.append(f"{tool.upper()} (ZAPIER) ERROR:\n{error_msg}")
                        logger.warning(f"⚠️ Zapier tool error: {error_msg}")
                    continue
